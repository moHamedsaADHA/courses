...
export const getQuizzesByGradeOne = async (req, res, next) => {
  try {
    console.log('📚 جلب كويزات الصف الأول الثانوي...');
    ...
    console.log(`✅ تم جلب ${quizzes.length} كويز للصف الأول من أصل ${totalQuizzes}`);

    // إحصائيات الكويزات للصف الأول
    ...
    const recentQuizzes = await Quiz.find({
      ...
      .sort({ createdAt: -1 })
      .limit(3)
      .select('title createdAt totalQuestions subject')
      .populate('createdBy', 'name');

    res.status(200).json({
      success: true,
      message: 'تم جلب كويزات الصف الأول الثانوي بنجاح',
      grade: grade,
      data: quizzes,
      pagination: {
        currentPage: page,
    ...
    console.error('❌ خطأ في جلب كويزات الصف الأول:', error);
    next(error);
  }
};