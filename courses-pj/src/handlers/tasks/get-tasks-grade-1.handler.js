...
export const getTasksByGradeOne = async (req, res, next) => {
  try {
    console.log('📚 جلب مهام الصف الأول الثانوي...');
    ...
    let sortOption = { 
      priority: { 'عاجل': 4, 'عالي': 3, 'متوسط': 2, 'منخفض': 1 },
      dueDate: 1, 
      createdAt: -1 
    };
    ...
    console.log('🔍 فلاتر البحث للصف الأول:', filters);
    ...
    const [tasks, totalTasks] = await Promise.all([
      Task.find(filters)
        ...
        .sort(req.query.sortBy ? sortOption : { dueDate: 1, createdAt: -1 })
        .skip(skip)
        .limit(limit),
      Task.countDocuments(filters)
    ]);

    console.log(`✅ تم جلب ${tasks.length} مهمة للصف الأول من أصل ${totalTasks}`);

    // إحصائيات المهام للصف الأول
    ...
    res.status(200).json({
      success: true,
      message: 'تم جلب مهام الصف الأول الثانوي بنجاح',
      grade: grade,
      data: tasks,
      pagination: {
        currentPage: page,
    ...
    console.error('❌ خطأ في جلب مهام الصف الأول:', error);
    next(error);
  }
};