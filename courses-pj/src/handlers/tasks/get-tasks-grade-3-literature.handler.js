...
export const getTasksByGradeThreeLiterature = async (req, res, next) => {
  try {
    console.log('📖 جلب مهام الصف الثالث الثانوي أدبي...');
    ...
    console.log(`✅ تم جلب ${tasks.length} مهمة للصف الثالث أدبي من أصل ${totalTasks}`);
    ...
    res.status(200).json({
      success: true,
      message: 'تم جلب مهام الصف الثالث الثانوي أدبي بنجاح',
      grade: grade,
      data: tasks,
      pagination: {
        currentPage: page,
    ...
    console.error('❌ خطأ في جلب مهام الصف الثالث أدبي:', error);
    next(error);
  }
};