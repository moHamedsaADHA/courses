...
export const getTasksByGradeTwoLiterature = async (req, res, next) => {
  try {
    console.log('📚 جلب مهام الصف الثاني الثانوي أدبي...');
    ...
    console.log(`✅ تم جلب ${tasks.length} مهمة للصف الثاني أدبي من أصل ${totalTasks}`);

    // إحصائيات المهام للصف الثاني أدبي
    ...
    res.status(200).json({
      success: true,
      message: 'تم جلب مهام الصف الثاني الثانوي أدبي بنجاح',
      grade: grade,
      data: tasks,
      pagination: {
        currentPage: page,
    ...
    console.error('❌ خطأ في جلب مهام الصف الثاني أدبي:', error);
    next(error);
  }
};